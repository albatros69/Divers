...
def scrape_url(url):
    ...
    result = [ ]
    for div in BeautifulSoup(page.text, 'lxml').find_all('div', class_='view-events-by-day'):
        for row in div.find_all('div', class_='views-row'):
            location = ''.join(row.find('div', class_='views-field-field-field-nr-event-location').stripped_strings)
            if 'Strasbourg' in location:
                title = row.find('div', class_='views-field-title-field')
                tmp = {
                    'date-start': extract_date(row.find('div', class_='event-date-start')), 
                    'location':   location,
...
for d in scrape_url('https://europa.eu/newsroom/events/year_fr?field_nr_events_by_topic_tid=All&field_nr_event_source_tid=51&field_nr_event_type_tid=All'):
